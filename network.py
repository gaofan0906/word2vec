...
def get_vector(x, sess, embedding_size, vocab_size):
    ...
    print("word_vec[70]: %s" % word_vec[70])
    # 0-69
    x_vector = []
    for arr in x:
        sample = []
        for i in arr:
            sample.append(word_vec[i - 1])
        x_vector.append(sample)
    print("x_vector shape:")
    print(np.array(x_vector).shape)
    print(np.array(x_vector))
    # 文本形式保存，只能保存一二维
    # np.savetxt("./data/output/vector",x_vector)
    # 可以保存高维
    np.save(file="./data/output/vector_x.npy", arr=x_vector)

    # with tf.variable_scope("embedding", reuse=True):
    #     W = tf.get_variable("weights")
    #     print("init weight name: {}".format(W.name))
    #     sess.run(W.assign(np.array(x_vector)))
...
def embedding(input_x, vocab_size, embedding_size, name):
    with tf.variable_scope(name):  # 管理传给的变量get_variable名称的作用域
        W = tf.get_variable("weights", [vocab_size, embedding_size])  # 创建或返回给定名称的变量
...
def conv(input, k_h, k_w, c_i, c_o, s_h, s_w, name, relu=True, padding=DEFAULT_PADDING):
    with tf.variable_scope(name) as scope:
        ...
        W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="weights") # 产生正太分布随机数
